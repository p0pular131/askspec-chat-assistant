...
const ChatConversationList: React.FC<ChatConversationListProps> = ({
  ...
  const renderConversationsList = useCallback(() => {
    ...
    return conversations.map((convo) => {
      const isUpdating = updatingSessionId === convo.id;
      
      return (
        <div key={convo.id} className="flex items-center gap-2">
          <button
            className={`p-2 w-full text-sm text-left rounded text-neutral-700 hover:bg-neutral-100 transition-all duration-300 ${
              currentConversation?.id === convo.id ? 'bg-neutral-100 font-medium' : ''
            } ${
              isUpdating ? 'bg-yellow-100 animate-pulse border-2 border-yellow-300' : ''
            }`}
  ...
  }, [conversations, currentConversation, loading, updatingSessionId, onSelect, handleDelete]);
