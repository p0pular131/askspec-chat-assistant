...
const ChatConversationList: React.FC<ChatConversationListProps> = ({
  ...
  const renderConversationsList = useCallback(() => {
    ...
    return conversations.map((convo) => {
      const isUpdating = titleUpdatingSessionId === convo.id;
      
      return (
        <div key={convo.id} className="flex items-center gap-2">
          <button
            className={`p-2 w-full text-sm text-left rounded text-neutral-700 hover:bg-neutral-100 transition-all duration-200 ${
              currentConversation?.id === convo.id ? 'bg-neutral-100 font-medium' : ''
            } ${
              isUpdating ? 'animate-pulse bg-blue-50 border-2 border-blue-200' : ''
            }`}
            onClick={() => onSelect(convo)}
          >
            <span className={isUpdating ? 'text-blue-700 font-medium' : ''}>
              {convo.session_name || 'Untitled conversation'}
            </span>
            {isUpdating && (
              <div className="text-xs text-blue-600 mt-1">제목이 생성되었습니다!</div>
            )}
          </button>
          <button
  ...
  }, [conversations, currentConversation, loading, titleUpdatingSessionId, onSelect, handleDelete]);
